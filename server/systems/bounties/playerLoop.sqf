...
{
_missionHintText = format ["<br/><t align='center' color='%2'>------------------------------</t><br/><t color='%3' size='1.0'>The coward %1 on %4 has reconnected and a bounty is still on his head. Let's hope he doesn't run away like a pussy again! You have 30 minutes to kill him! Killer gets $100,000 and his side gets $5,000 per person. If he's protected he gets the $100,000 and his side gets $5,000 per person.</t>", _targetName, bountyMissionColor, subTextColor, _targetSideName];
_missionType = "Coward Bounty";
};

if(_BountyType == 'SYSTEM') then
{
_missionHintText = format ["<br/><t align='center' color='%2'>------------------------------</t><br/><t color='%3' size='1.0'>%1 on %4 has a bounty on his head. You have 30 minutes to kill him! Killer gets $100,000 and his side gets $5,000 per person. If he's protected he gets the $100,000 and his side gets $5,000 per person.</t>", _targetName, bountyMissionColor, subTextColor, _targetSideName];
_missionType = "Bounty";
};
...
[format ["addBounty:%1:%2:%3:%4:%5:%6:%7:%8", call A3W_extDB_ServerID, call A3W_extDB_MapID, _BountyType, _targetUID, _sourceUID, _mission_state, 5000, 100000]] call extDB_Database_async;
...
private ["_bountyDeath","_xMarkerOffset","_yMarkerOffset"];

_xMarkerOffset = random 50;
_yMarkerOffset = random 50;
...
_lastKnownPosition = position _targetPlayer;

_targetMarker setMarkerPos [(_lastKnownPosition select 0) + _xMarkerOffset, (_lastKnownPosition select 1) + _yMarkerOffset];
...
if (_mission_state == BOUNTY_MISSION_END_SURVIVED) then {
		...
		_playerMoney = _playerMoney + 100000;
		...
		_failedHintMessage = format ["<br/><t align='center' color='%2'>------------------------------</t><br/><t align='center' color='%2' size='1.25'>%1 lives!</t><br/><br/><t align='center' color='%3'>%1 gets $100,000 and every member of %4 get $5,000!</t>", _targetName, failMissionColor, subTextColor, _targetSideName];
	};
	...
	if (_mission_state == BOUNTY_MISSION_END_SUICIDE) then {
		...
		[format ["updateBounty:%1:%2:%3:%4", '0','0', _mission_state, _targetUID]] call extDB_Database_async;
		[format ["setPlayerBountyDisabled:%1", _targetUID]] call extDB_Database_async;
	};
	...
	if (_mission_state == BOUNTY_MISSION_END_KILLED) then 
	{
		...
		_successHintMessage = format ["<br/><t align='center' color='%2'>------------------------------</t><br/><t align='center' color='%6' size='1.25'>%1</t><br/><t align='center'><img size='5' image='%7'/></t><br/><t align='center' color='%3'>%1 has been killed by %4! %5 has earned $5,000 for each member and %4 has earned $100,000!</t>", _targetName, successMissionColor, subTextColor, _killerName, _killerSideName, failMissionColor,_missionPicture];
